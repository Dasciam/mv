...
func (*InventoryTransaction) ID() uint32 {
	return gtpacket.IDInventoryTransaction
}
...
func (pk *InventoryTransaction) Marshal(io protocol.IO) {
	...
	if _, ok := io.(*protocol.Reader); ok {
		...
		if !v686protocol.LookupTransactionData(transactionType, &pk.TransactionData) {
			io.UnknownEnumOption(transactionType, "inventory transaction data type")
		}
	} else {
		var id uint32
		if !v686protocol.LookupTransactionDataType(pk.TransactionData, &id) {
			io.UnknownEnumOption(fmt.Sprintf("%T", pk.TransactionData), "inventory transaction data type")
		}
		io.Varuint32(&id)
	}
